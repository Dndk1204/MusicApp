...
const HoverImagePreview = ({ 
    ...
    return (
        <>
            ...
            {isHovering && !isMobile && createPortal(
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div className={`relative w-full h-full bg-black border-t-2 border-l-2 p-1 shadow-[10px_10px_0px_0px_rgba(16,185,129,0.2)] ${status === "error" ? "border-red-500" : "border-emerald-500"}`}>
                            <div className="relative w-full h-full overflow-hidden bg-neutral-900">
                                {src ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center">
                                        <Disc size={40} className="text-emerald-900 animate-spin-slow" />
                                    </div>
                                )}
                                <ScanlineOverlay />
                                {status === "playing" && (
                                    <div className="absolute bottom-2 left-2 flex items-end gap-[2px] h-10 z-30">
                                        {[...Array(12)].map((_, i) => (
                                            <motion.div key={i} className="w-[3px] bg-emerald-500/80" animate={{ height: [4, Math.random() * 30 + 5, 4] }} transition={{ repeat: Infinity, duration: 0.4, delay: i * 0.05 }} />
                                        ))}
                                    </div>
                                )}
                            </div>
                            {/* CYBER HEADER */}
                            <div className="absolute top-0 left-0 w-full p-2 flex items-center justify-between font-mono z-40 bg-black/60 backdrop-blur-sm border-b border-white/10 text-[9px]">
                                <div className="flex items-center gap-2">
                                    <Activity size={12} className="text-emerald-500 animate-pulse" />
                                    <span className="text-white font-bold uppercase tracking-widest">PREVIEW_MODE</span>
                                </div>
                                <span className="text-emerald-500/70">20S_MAX</span>
                            </div>
                        </div>
                    </motion.div>
                </AnimatePresence>,
