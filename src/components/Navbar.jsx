...
import { User, LogOut, LogIn, UserPlus, ShieldCheck, Settings, Search, Disc, Sun, Moon, SlidersHorizontal, X } from "lucide-react"; 
...
const Navbar = () => {
  ...
  return (
    ...
    ">
      ...
      {/* LEFT: LOGO (VOID - Original Style with Square tweak) */}
      <div className="flex items-center gap-x-6">
        <div 
          className="hidden md:flex items-center gap-x-4 cursor-pointer group" 
          onClick={() => router.push('/')}
        >
          {/* THE PORTAL ICON (Vuông vức hóa) */}
          <div className="relative w-10 h-10 bg-neutral-900 dark:bg-black flex items-center justify-center overflow-hidden border border-neutral-400 dark:border-white/20 group-hover:border-emerald-500 transition-colors duration-300">
          ...
          {/* VOID TYPOGRAPHY (Giữ nguyên hiệu ứng Glitch cũ) */}
      ...
      {/* CENTER: SEARCH BAR (TECH INPUT - Cyber-Brutalism) */}
      ...
      {/* RIGHT: PROFILE & MENU (SQUARE AVATAR) */}
               ...
               {user ? (
                <>
                  ...
                  <div className="p-2 space-y-1">
                      <div onClick={() => { router.push('/profile'); setShowMenu(false); }} className="px-3 py-2 text-xs font-mono uppercase text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-white/10 hover:text-black dark:hover:text-white cursor-pointer flex items-center gap-3 transition-colors border border-transparent hover:border-neutral-300 dark:hover:border-white/20">
                        <User size={14} /> My_Profile
                      </div>

                      <div onClick={() => { router.push('/account'); setShowMenu(false); }} className="px-3 py-2 text-xs font-mono uppercase text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-white/10 hover:text-black dark:hover:text-white cursor-pointer flex items-center gap-3 transition-colors border border-transparent hover:border-neutral-300 dark:hover:border-white/20">
                        <Settings size={14} /> Config_Account
                      </div>
