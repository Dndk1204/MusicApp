...
import { 
  ...
  Globe, Lock, Star, ArrowDownWideNarrow, ArchiveRestore, Skull 
} from "lucide-react";
import useUI from "@/hooks/useUI";
import useUploadModal from "@/hooks/useUploadModal"; 
// --- THÊM IMPORT UploadModal ---
...
const AdminDashboard = () => {
  ...
  // --- FIX LỖI: Safe destructuring với fallback ---
  const ui = useUI() || {}; 
  const success = ui.success || ((msg) => alert(`SUCCESS: ${msg}`));
  const error = ui.error || ((msg) => alert(`ERROR: ${msg}`));
  const confirm = ui.confirm || ((msg) => window.confirm(msg));
  ...
  // --- HELPER: Lấy thông tin người đăng ---
  const getUploaderInfo = (userId) => {
      if (!userId) return { name: 'System', role: 'admin' };
      ...
      return { name: 'Deleted User', role: 'unknown' };
  };

  // --- HELPER: Kiểm tra bài hát của Admin ---
  ...
  const handleSyncMusic = async () => { 
    ...
    try { 
        ...
        const fetchPromises = offsets.map(offset => fetch(`https://api.jamendo.com/v3.0/tracks/?client_id=${CLIENT_ID}&format=jsonpretty&limit=20&include=musicinfo&order=popularity_week&offset=${offset}`).then(res => res.json())); 
        const responses = await Promise.all(fetchPromises); 
        responses.forEach(data => { if (data.results) allTracks = [...allTracks, ...data.results]; }); 
        
        if (allTracks.length > 0) { 
            const songsToInsert = allTracks.map(track => ({ title: track.name, author: track.artist_name, song_url: track.audio, image_url: track.image, duration: track.duration, play_count: 0, is_public: true }));
            const { error: upsertError } = await supabase.from('songs').upsert(songsToInsert, { onConflict: 'song_url', ignoreDuplicates: true }); 
            if (upsertError) throw upsertError; 
            
            success("Synced 100 tracks successfully from API!"); 
            await fetchDashboardData(); 
        } 
    } catch (e) { 
        error(e.message); 
  ...
  const handleSyncArtists = async () => { 
    if (!await confirm("Update data for top 50 artists?", "SYNC_ARTISTS")) return; 
    ...
    try { 
        ...
        if (data.results) { 
            ...
            const { error: upsertError } = await supabase.from('artists').upsert(artistsToUpsert, { onConflict: 'name', ignoreDuplicates: true }); 
            if (upsertError) throw upsertError; 
            
            success("Top 50 artists data synced!"); 
            await fetchDashboardData(); 
        } 
    } catch (e) { 
        error(e.message); 
  ...
  const handleResetArtists = async () => { 
    if (!await confirm("Reset DB? This action cannot be undone.", "CONFIRM_RESET")) return; 
    setResetting(true); 
    try { 
        await supabase.rpc('reset_artists_data'); 
        success("Artists database reset successfully."); 
        await fetchDashboardData(); 
    } catch (e) { 
        error(e.message); 
    } finally { 
  ...
  const handleRestoreFollowed = async () => { 
    if (!await confirm("Restore followed artists from user logs?", "CONFIRM_RESTORE")) return; 
    setRestoring(true); 
    try { 
        await supabase.rpc('restore_followed_artists'); 
        success("Followed artists restored successfully."); 
        await fetchDashboardData(); 
    } catch (e) { 
        error(e.message); 
    } finally { 
  ...
  const handleCleanupSongs = async () => { 
    if (!await confirm("Remove duplicate songs?", "CLEANUP_WARNING")) return; 
    setCleaning(true); 
    try { 
        await supabase.rpc('cleanup_duplicate_songs'); 
        success("Duplicate songs removed successfully."); 
        await fetchDashboardData(); 
    } catch (err) { 
        error(err.message); 
    } finally { 
  ...
  const handleCleanupArtists = async () => { 
    if (!await confirm("Remove duplicate artists?", "CLEANUP_WARNING")) return; 
    setCleaning(true); 
    try { 
        await supabase.rpc('cleanup_duplicate_artists'); 
        success("Duplicate artists removed successfully."); 
        await fetchDashboardData(); 
    } catch (err) { 
        error(err.message); 
    } finally { 
  ...
  const handleDeleteUser = async (id) => { 
      if(await confirm("Delete this user permanently?", "DELETE_USER")) { 
  ...
  const handleDeleteSong = async (id) => { 
      if(await confirm("Remove this track permanently?", "DELETE_SONG")) { 
  ...
  const handleDeleteSearch = async (name) => { 
      if(await confirm(`Clear history for "${name}"?`, "DELETE_LOG")) { 
          await supabase.from('artist_search_counts').delete().eq('artist_name', name); 
          success("Search log cleared.");
          fetchDashboardData(); 
      } 
  };
  ...
  const handleDeleteDbArtist = async (id) => { 
      if (!id) return; 
      if(await confirm("Remove artist from database?", "DELETE_ARTIST")) { 
  ...
  return (
    <div className="h-full w-full p-6 pb-[120px] overflow-y-auto bg-neutral-100 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-200 transition-colors duration-500 relative">
      
      {/* --- HIỂN THỊ MODAL UPLOAD TẠI ĐÂY --- */}
      <UploadModal />
        ...
        <div className="animate-in fade-in zoom-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="group bg-white dark:bg-white/5 border border-neutral-200 dark:border-white/10 backdrop-blur-xl rounded-2xl p-6 hover:border-purple-500/50 shadow-sm dark:shadow-none transition-all">
                    <h3 className="text-neutral-900 dark:text-white font-mono text-lg mb-2 group-hover:text-purple-600 dark:group-hover:text-purple-400">DB & UPLOADS</h3>
      ...
      {/* VIEW: SONG TABLES (DÙNG CHUNG CHO: ALL, ADMIN, USER) */}
      {isSongTableView && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                    ...
                    {currentView === 'songs_list' && (
                         <GlitchButton onClick={handleCleanupSongs} disabled={cleaning} className="bg-red-500/10 border-red-500/50 text-red-600 dark:text-red-400 px-4 py-2 text-xs">{cleaning ? <Loader2 className="animate-spin" size={14}/> : <Eraser size={14}/>} CLEANUP</GlitchButton>
                    )}
            ...
            <div className="bg-white dark:bg-black/20 border border-neutral-200 dark:border-white/5 rounded-xl overflow-hidden backdrop-blur-sm">
                ...
                <div className="overflow-x-auto max-h-[600px]">
                    <table className="w-full text-left text-xs font-mono text-neutral-600 dark:text-neutral-400">
                        <thead className="bg-neutral-100 dark:bg-black/40 text-neutral-500 uppercase tracking-widest sticky top-0 z-10 backdrop-blur-md">
                            <tr>
                                ...
                                <th className="px-6 py-4">Plays <ArrowDownWideNarrow size={12} className="inline"/></th>
                                ...
                                return (
                                    <tr key={song.id} className="hover:bg-neutral-50 dark:hover:bg-white/5 transition">
                                        <td className="px-6 py-4 flex items-center gap-3 align-middle">
                                            <div className="w-8 h-8 rounded bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-white/10 overflow-hidden flex-shrink-0">
                                                {song.image_url ? <img src={song.image_url} className="w-full h-full object-cover"/> : <Music size={12}/>}
                                            </div>
                                            <span className="text-neutral-800 dark:text-neutral-200 truncate max-w-[150px]">{song.title}</span>
                                        </td>
                                        <td className="px-6 py-4 text-neutral-600 dark:text-neutral-400 align-middle">{song.author}</td>
                                        <td className="px-6 py-4 align-middle">
                                            <span className={`text-[9px] px-2 py-0.5 rounded border ${uploader.role === 'admin' ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-500' : 'bg-blue-500/10 border-blue-500/30 text-blue-500'}`}>
                                                {uploader.name}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 align-middle">
                                            {song.public ?
                                                <span className="flex items-center gap-1 text-blue-500"><Globe size={12}/> Public</span> :
                                                <span className="flex items-center gap-1 text-red-500"><Lock size={12}/> Private</span>
                                            }
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right align-middle">
                                            <button onClick={() => handleDeleteSong(song.id)} className="text-neutral-500 hover:text-red-500 transition p-2 hover:bg-red-500/10 rounded"><Trash2 size={16} /></button>
                                        </td>
                                    </tr>
                                );
                            })}
                    ...
                    {filteredSongs.length === 0 && <div className="p-8 text-center text-neutral-500 font-mono text-xs">[NO_TRACKS_FOUND]</div>}
                </div>
            </div>
        </div>
      )}
                        ...
                        <tbody className="divide-y divide-neutral-200 dark:divide-white/5">
                            {filteredSearchLogs.map((artist, i) => (
                                <tr key={i} className="hover:bg-neutral-50 dark:hover:bg-white/5">
                                    <td className="px-6 py-4 align-middle"><span className="text-neutral-800 dark:text-neutral-200 font-bold">{artist.artist_name}</span></td>
                                    <td className="px-6 py-4 text-blue-600 dark:text-blue-400 align-middle">{artist.search_count}</td>
                                    <td className="px-6 py-4 text-right align-middle"><button onClick={() => handleDeleteSearch(artist.artist_name)} className="hover:text-red-500"><Trash2 size={16} /></button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
      ...
      {currentView === 'db_artists_list' && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-4">
                <button onClick={() => { setCurrentView('dashboard'); setArtistSearchTerm(""); }} className="flex items-center gap-2 text-neutral-500 dark:text-neutral-400 hover:text-black dark:hover:text-white text-xs font-mono"><ArrowLeft size={14}/> BACK</button>
